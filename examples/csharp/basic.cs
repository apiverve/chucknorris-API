...
{
    class ChuckNorrisJokesExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/chucknorris";

        /// <summary>
        /// Make a GET request to the Chuck Norris Jokes API
        /// </summary>
        static async Task<JsonDocument> CallChuckNorrisJokesAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Chuck Norris Jokes API...\n");

            var result = await CallChuckNorrisJokesAPI();
